...
LmxSpiInterface::LmxSpiInterface(int CSB, int SCK, int SDI_DATA_OUT, int MUX, int CE){
	_CSB = CSB;
	_SCK = SCK;
	_SDI_DATA_OUT = SDI_DATA_OUT;
	_MUX = MUX;
	_CE = CE;
	pinMode(_CSB, OUTPUT);
	pinMode(_SCK, OUTPUT);
	pinMode(_SDI_DATA_OUT, OUTPUT);
	pinMode(_MUX, INPUT);
	pinMode(_CE, OUTPUT);
...
void LmxSpiInterface::init_registers(){
...
void LmxSpiInterface::writeBits(uint32_t word, uint8_t nbits)
{
	//Serial.print(nbits);
	//Serial.print(" ");
	...
	for(uint32_t b = nbits ; b > 0 ; b--){
		...
		if(word & 0x80000000){
			digitalWrite(_SDI_DATA_OUT, HIGH);
			//Serial.print("1");
		}
		else{
			digitalWrite(_SDI_DATA_OUT, LOW);
			//Serial.print("0");
		}
...
uint16_t LmxSpiInterface::readBits(uint8_t nbits)
{
	uint16_t data=0;
	for(uint32_t b = nbits ; b > 0 ; b--){
		digitalWrite(_SCK, LOW);
		data <<= 1;
		data |= digitalRead(_MUX);
		digitalWrite(_SCK, HIGH);
	}
	return data;
}
...
void LmxSpiInterface::sendWord(uint32_t word)
{
...
uint16_t LmxSpiInterface::receiveWord(uint32_t word){
	uint16_t d;
	digitalWrite(_CSB, LOW);
	writeBits(word, 8);
	d = readBits(16);
...
uint16_t LmxSpiInterface::readRegister(uint32_t addr){
	addr &= 0x7F;
	addr |= 0x80;
	return receiveWord(addr<<24);
}

void LmxSpiInterface::writeRegister(uint32_t addr, uint32_t data){
	addr &= 0x7F;
	data &= 0xFFFF;
